        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <?php if ($activeYearLabel !== null): ?>
                <p class="mt-2 text-sm text-slate-600">
                    Data yang dibersihkan akan difilter berdasarkan tahun ajaran aktif berikut.
                </p>
            ...
            <?php else: ?>
                <div class="mt-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
                    Belum ada tahun ajaran yang berstatus aktif. Aktifkan tahun ajaran terlebih dahulu sebelum menjalankan pembersihan data raport.
                </div>
            <?php endif; ?>

            <div class="mt-6 rounded-xl border border-amber-100 bg-amber-50 px-4 py-3 text-xs text-amber-700">
                <p class="font-semibold text-amber-800">Catatan penting</p>
                <ul class="mt-2 space-y-1 list-disc pl-5">
                    <li>Data yang dihapus tidak dapat dikembalikan. Pastikan sudah melakukan backup.</li>
                    <li>Pembersihan hanya mencakup data akademik (raport) pada tahun ajaran aktif.</li>
                    <li>Gunakan fitur ini sebelum memulai input nilai untuk semester baru.</li>
                </ul>
            </div>
        </div>
    ...
    <div class="space-y-6 xl:col-span-8">
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-6 py-4">
                <h2 class="text-base font-semibold text-slate-800">Pilih Data Raport yang Akan Dibersihkan</h2>
            ...
            <form
                action="<?= htmlspecialchars(base_url('admin/clean-data/raport'), ENT_QUOTES, 'UTF-8') ?>"
                method="post"
                class="divide-y divide-slate-100"
                data-clean-rapor-form
            >
                <?= csrf_field() ?>
                            ...
                            <?php if (empty($definitions)): ?>
                                <tr>
                                    <td colspan="3" class="px-6 py-5 text-center text-sm text-slate-400">
                                        Belum ada konfigurasi dataset untuk pembersihan raport.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                ...
                <div class="px-6 py-5 space-y-4">
                    ...
                    <div class="flex items-start gap-3 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
                        ...
                        <p>Langkah ini bersifat permanen dan akan menghapus data raport sesuai pilihan Anda.</p>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    var form = document.querySelector('[data-clean-rapor-form]');
    if (!form) {
        return;
    }
