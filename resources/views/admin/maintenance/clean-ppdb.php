<?php
    ...
    $targetYearData = is_array($targetYear ?? null) ? $targetYear : null;
...
<div class="grid gap-6 xl:grid-cols-12">
    <div class="space-y-6 xl:col-span-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 class="text-base font-semibold text-slate-800">Target Tahun Ajaran PPDB</h2>
            <p class="mt-2 text-sm text-slate-600">
                Pilih tahun ajaran target PPDB yang datanya ingin dibersihkan. Periode, pendaftar, dan pembayaran akan difilter berdasarkan pilihan ini.
            </p>

            <?php if (!empty($yearOptions)): ?>
                <form
                    action="<?= htmlspecialchars(base_url('admin/clean-data/ppdb'), ENT_QUOTES, 'UTF-8') ?>"
                    method="get"
            ...
            <?php else: ?>
                <div class="mt-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
                    Belum ada data tahun ajaran yang tersedia. Tambahkan tahun ajaran terlebih dahulu pada menu Master › Tahun Ajaran.
                </div>
            <?php endif; ?>
            ...
            <div class="mt-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-700">
                ...
                <ul class="mt-2 space-y-1 list-disc pl-5">
                    <li>Pastikan telah melakukan backup database sebelum menjalankan pembersihan.</li>
                    <li>Pembersihan hanya berdampak pada periode PPDB yang menargetkan tahun ajaran terpilih.</li>
                    <li>Langkah ini tidak dapat dibatalkan. Gunakan hanya ketika memulai siklus PPDB baru.</li>
                </ul>
            </div>
        </div>
    ...
    <div class="space-y-6 xl:col-span-8">
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-6 py-4">
                <h2 class="text-base font-semibold text-slate-800">Pilih Data PPDB yang Ingin Dibersihkan</h2>
                <p class="mt-1 text-sm text-slate-500">
                    Centang jenis data yang akan dihapus, lalu ketik <span class="font-semibold text-slate-700">BERSIHKAN</span> untuk mengonfirmasi.
                </p>
            </div>
            ...
            <form
                ...
                data-clean-ppdb-form
            >
                <?= csrf_field() ?>
                <input type="hidden" name="target_year_id" value="<?= $selectedYearId > 0 ? (int) $selectedYearId : 0 ?>" />

                <div class="flex flex-wrap gap-3 px-6 py-4 text-xs font-semibold uppercase tracking-wide text-slate-500">
                            ...
                            <?php if (empty($definitions)): ?>
                                <tr>
                                    <td colspan="3" class="px-6 py-5 text-center text-sm text-slate-400">
                                        Belum ada konfigurasi dataset untuk pembersihan PPDB.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                ...
                <div class="space-y-4 px-6 py-5">
                    ...
                    <div class="flex items-start gap-3 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
                        ...
                        <p>Pembersihan data PPDB bersifat permanen dan tidak dapat dibatalkan.</p>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    var form = document.querySelector('[data-clean-ppdb-form]');
    if (!form) {
        return;
    }
