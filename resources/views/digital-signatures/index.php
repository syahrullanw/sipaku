...
<div class="space-y-6">
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div class="flex items-start justify-between gap-2">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500">Total dokumen: <?= htmlspecialchars((string) $totalCount, ENT_QUOTES, 'UTF-8') ?></p>
                                </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex flex-col gap-4 border-b border-slate-100 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h3 class="text-base font-semibold text-slate-800">Daftar Dokumen</h3>
                <p class="text-sm text-slate-500">Dokumen yang membutuhkan persetujuan kepala sekolah untuk menampilkan TTD digital.</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <?php foreach ($statusLabels as $key => $label): ?>
                    <?php $isActive = $statusFilter === $key; ?>
                    <a
                        href="<?= htmlspecialchars(base_url('kepala-sekolah/ttd-digital?status=' . urlencode((string) $key)), ENT_QUOTES, 'UTF-8') ?>"
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        <?php foreach ($records as $record): ?>
                            <?php
                                ...
                                $semesterInfo = isset($payload['semester']) ? (int) $payload['semester'] : null;
                            ?>
                            <tr>
                                <td class="px-6 py-4 align-top">
                                    ...
                                    <?php if ($semesterInfo !== null): ?>
                                        <p class="mt-1 text-xs text-slate-400">Semester: <?= htmlspecialchars((string) $semesterInfo, ENT_QUOTES, 'UTF-8') ?></p>
                                    <?php endif; ?>
