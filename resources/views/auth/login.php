<?php
    ...
    $publicAccessLinks = [
        ...
        [
            'label' => 'Verifikasi Kartu Pelajar',
            'href' => base_url('kartu-pelajar/verifikasi'),
            'icon' => 'card',
        ],
        [
            'label' => 'Verifikasi Dokumen',
            'href' => base_url('dokumen/validasi'),
            'icon' => 'document',
        ],
        [
            'label' => 'Verifikasi Persuratan',
            'href' => base_url('persuratan/validasi'),
            'icon' => 'letter',
        ],
    ];

    if (!empty($ppdbRegistrationLink)) {
        array_unshift($publicAccessLinks, [
            'label' => 'Formulir PPDB',
            'href' => (string) $ppdbRegistrationLink,
            'icon' => 'document',
        ]);
    }
?>

<section class="animated-login-shell">
    <header class="text-center">
        <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-white shadow">
            <img
        ...
        <p class="text-slate-500 mt-1 text-sm">
            Masuk untuk mengelola administrasi <?= htmlspecialchars(config('app.name'), ENT_QUOTES, 'UTF-8') ?>.
        </p>
    </header>

    <?php if (!empty($success)): ?>
        <div class="alert mb-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
            <?= htmlspecialchars($success, ENT_QUOTES, 'UTF-8') ?>
        </div>
    <?php endif; ?>

    <?php if (!empty($error)): ?>
        <div class="alert mb-4 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
            <?= htmlspecialchars($error, ENT_QUOTES, 'UTF-8') ?>
        </div>
    <?php endif; ?>

    <form action="<?= htmlspecialchars(base_url('login'), ENT_QUOTES, 'UTF-8') ?>" method="post" class="animated-login" novalidate>
        <?= csrf_field() ?>
        <div class="svgContainer" aria-hidden="true">
            <div>
        ...
        <div class="inputGroup inputGroup1<?= $oldUsername !== '' ? ' focusWithText' : '' ?>">
            <label for="loginIdentifier" id="loginIdentifierLabel">Kredensial</label>
            <input
                type="text"
                id="loginIdentifier"
                name="username"
            ...
            />
            <p class="helper helper1">username / email / NIP / NIS / NISN</p>
        </div>
        ...
        <nav class="public-shortcuts" aria-label="Akses publik">
            <span class="public-shortcuts-label">Akses publik</span>
            <div class="public-shortcuts-list">
            <?php foreach ($publicAccessLinks as $link): ?>
                <a
                    class="public-shortcut"
                    href="<?= htmlspecialchars($link['href'], ENT_QUOTES, 'UTF-8') ?>"
                    title="<?= htmlspecialchars($link['label'], ENT_QUOTES, 'UTF-8') ?>"
                    aria-label="<?= htmlspecialchars($link['label'], ENT_QUOTES, 'UTF-8') ?>"
                >
                    <?php if ($link['icon'] === 'graduation'): ?>
                        <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                            <path d="M22 10 12 5 2 10l10 5 10-5Z"></path>
                            <path d="M6 12v4c0 1.7 2.7 3 6 3s6-1.3 6-3v-4"></path>
                            <path d="M22 10v6"></path>
                        </svg>
                    <?php elseif ($link['icon'] === 'card'): ?>
                        <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                            <rect x="3" y="5" width="18" height="14" rx="2"></rect>
                            <circle cx="9" cy="11" r="2"></circle>
                            <path d="M6.5 16c.7-1.4 4.3-1.4 5 0"></path>
                            <path d="M14 10h4"></path>
                            <path d="M14 14h4"></path>
                        </svg>
                    <?php elseif ($link['icon'] === 'document'): ?>
                        <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
