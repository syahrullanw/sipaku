...
<div class="space-y-6">
    <div class="rounded-xl border border-slate-200/60 bg-white/80 p-6 shadow-sm shadow-slate-100 dark:border-slate-700/70 dark:bg-slate-900/60 dark:shadow-none">
        <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    Lengkapi laporan pertanggungjawaban atas dana tak terduga yang Anda terima. Sertakan bukti pengeluaran bila memungkinkan.
                </p>
            </div>
        ...
        <dl class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div>
                <dt class="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Pemohon</dt>
    ...
    <div class="rounded-xl border border-slate-200/60 bg-white/80 p-6 shadow-sm shadow-slate-100 dark:border-slate-700/70 dark:bg-slate-900/60 dark:shadow-none">
        <form method="post" action="<?= htmlspecialchars(base_url('keuangan/guru/pengeluaran-tak-terduga/' . (int) ($expense['id'] ?? 0) . '/lpj'), ENT_QUOTES, 'UTF-8') ?>" enctype="multipart/form-data" class="space-y-5">
            <?= csrf_field() ?>
