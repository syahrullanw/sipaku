...
$currentAmount = old('nominal', isset($report['nominal']) ? (string) $report['nominal'] : (string) ($activity['estimasi_biaya'] ?? '0'));
...
<div class="space-y-6">
    <div class="rounded-xl border border-slate-200/60 bg-white/80 p-6 shadow-sm shadow-slate-100 dark:border-slate-700/70 dark:bg-slate-900/60 dark:shadow-none">
        <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
                <h1 class="text-lg font-semibold text-slate-900 dark:text-white">LPJ Dana Kegiatan</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    Lengkapi laporan pertanggungjawaban setelah kegiatan dilaksanakan. Sertakan bukti penggunaan dana bila tersedia.
                </p>
            </div>
            <div class="rounded-lg border border-slate-200/70 bg-white px-4 py-3 text-sm dark:border-slate-700/70 dark:bg-slate-900/80">
                <p class="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Kode Pengajuan</p>
                <p class="mt-1 font-semibold text-slate-900 dark:text-white"><?= htmlspecialchars((string) ($activity['kode'] ?? '-'), ENT_QUOTES, 'UTF-8') ?></p>
            </div>
        </div>
        <dl class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div>
                <dt class="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Judul Kegiatan</dt>
                <dd class="mt-1 font-medium text-slate-900 dark:text-white"><?= htmlspecialchars((string) ($activity['judul'] ?? '-'), ENT_QUOTES, 'UTF-8') ?></dd>
            </div>
            <div>
                <dt class="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Kategori</dt>
                <dd class="mt-1 font-medium text-slate-900 dark:text-white"><?= htmlspecialchars((string) ($activity['kategori'] ?? '-'), ENT_QUOTES, 'UTF-8') ?></dd>
            </div>
            <div>
                <dt class="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Estimasi Biaya</dt>
                <dd class="mt-1 font-medium text-slate-900 dark:text-white"><?= htmlspecialchars($formatCurrency((float) ($activity['estimasi_biaya'] ?? 0)), ENT_QUOTES, 'UTF-8') ?></dd>
            </div>
            <div>
                <dt class="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Status Pengajuan</dt>
                <dd class="mt-1 inline-flex items-center rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-300">
                    <?= htmlspecialchars(ucwords(str_replace('_', ' ', (string) ($activity['status'] ?? '-'))), ENT_QUOTES, 'UTF-8') ?>
                </dd>
            </div>
        </dl>
    </div>
    ...
    <div class="rounded-xl border border-slate-200/60 bg-white/80 p-6 shadow-sm shadow-slate-100 dark:border-slate-700/70 dark:bg-slate-900/60 dark:shadow-none">
        <form method="post" action="<?= htmlspecialchars(base_url('keuangan/guru/dana-kegiatan/' . (int) ($activity['id'] ?? 0) . '/lpj'), ENT_QUOTES, 'UTF-8') ?>" enctype="multipart/form-data" class="space-y-5">
            <?= csrf_field() ?>
