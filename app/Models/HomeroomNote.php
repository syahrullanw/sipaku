<?php

namespace App\Models;

use Core\Model;
use PDO;
use Throwable;

class HomeroomNote extends Model
{
    protected static ?string $table = 'catatan_walikelas';

    /**
     * @return array<int, string>
     */
    public static function byClass(int $classId, ?int $schoolYearId = null): array
    {
        if ($classId <= 0) {
            return [];
        }

        $conditions = ['kelas_id = :kelas_id'];
        $params = [':kelas_id' => $classId];

        if ($schoolYearId !== null && $schoolYearId > 0) {
            $conditions[] = 'tahun_ajaran_id = :tahun_ajaran_id';
            $params[':tahun_ajaran_id'] = $schoolYearId;
        }

        $sql = sprintf(
            'SELECT siswa_id, catatan FROM %s WHERE %s',
            static::table(),
            implode(' AND ', $conditions)
        );

        $statement = static::connection()->prepare($sql);

        if ($statement === false) {
            return [];
        }

        foreach ($params as $placeholder => $value) {
            $statement->bindValue($placeholder, $value, PDO::PARAM_INT);
        }

        if (!$statement->execute()) {
            return [];
        }

        $rows = $statement->fetchAll(PDO::FETCH_ASSOC);

        if ($rows === false) {
            return [];
        }

        $records = [];

        foreach ($rows as $row) {
            $studentId = (int) ($row['siswa_id'] ?? 0);
            if ($studentId <= 0) {
                continue;
            }

            $records[$studentId] = (string) ($row['catatan'] ?? '');
        }

        return $records;
    }

    /**
     * @param array<int, string> $notes
     */
    public static function saveMany(int $classId, int $schoolYearId, int $teacherId, array $notes): void
    {
        if ($classId <= 0 || $schoolYearId <= 0 || $teacherId <= 0) {
            return;
        }

        $connection = static::connection();
        $connection->beginTransaction();

        try {
            $insert = $connection->prepare(
                <<<SQL
INSERT INTO catatan_walikelas (
    tahun_ajaran_id,
    kelas_id,
    siswa_id,
    guru_id,
    catatan,
    created_at,
    updated_at
) VALUES (
    :tahun_ajaran_id,
    :kelas_id,
    :siswa_id,
    :guru_id,
    :catatan,
    :created_at,
    :updated_at
)
ON DUPLICATE KEY UPDATE
    guru_id = VALUES(guru_id),
    catatan = VALUES(catatan),
    updated_at = VALUES(updated_at)
SQL
            );

            if ($insert === false) {
                throw new \RuntimeException('Gagal menyiapkan pernyataan catatan wali kelas.');
            }

            $delete = $connection->prepare(
                <<<SQL
DELETE FROM catatan_walikelas
WHERE tahun_ajaran_id = :tahun_ajaran_id
  AND kelas_id = :kelas_id
  AND siswa_id = :siswa_id
SQL
            );

            if ($delete === false) {
                throw new \RuntimeException('Gagal menyiapkan penghapusan catatan wali kelas.');
            }

            $now = date('Y-m-d H:i:s');

            foreach ($notes as $studentId => $note) {
                $studentId = (int) $studentId;
                if ($studentId <= 0) {
                    continue;
                }

                if (!Student::isActiveId($studentId)) {
                    continue;
                }

                if (!is_string($note)) {
                    $note = '';
                }

                $note = trim($note);

                if ($note === '') {
                    $delete->bindValue(':tahun_ajaran_id', $schoolYearId, PDO::PARAM_INT);
                    $delete->bindValue(':kelas_id', $classId, PDO::PARAM_INT);
                    $delete->bindValue(':siswa_id', $studentId, PDO::PARAM_INT);
                    $delete->execute();
                    $delete->closeCursor();
                    continue;
                }

                $insert->bindValue(':tahun_ajaran_id', $schoolYearId, PDO::PARAM_INT);
                $insert->bindValue(':kelas_id', $classId, PDO::PARAM_INT);
                $insert->bindValue(':siswa_id', $studentId, PDO::PARAM_INT);
                $insert->bindValue(':guru_id', $teacherId, PDO::PARAM_INT);
                $insert->bindValue(':catatan', $note, PDO::PARAM_STR);
                $insert->bindValue(':created_at', $now);
                $insert->bindValue(':updated_at', $now);

                $insert->execute();
                $insert->closeCursor();
            }

            $connection->commit();
        } catch (Throwable $exception) {
            $connection->rollBack();
            throw $exception;
        }
    }
}
